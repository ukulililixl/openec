...
public class NameNodeService implements RpcNameNodeService, Sequencer {
    ...
    // XL add special file prefix for openec
	...
	public short createFile(RpcRequestMessage.CreateFileReq request, RpcResponseMessage.CreateFileRes response, RpcNameNodeState errorState) throws Exception {
        ...
        LOG.info("XL-NameNodeService::createFile:fileHash = " + fileHash.toString() + ", type = " + type.getLabel() + ", writeable = " + writeable + ", storageClass = " + storageClass + ", locationClass = " + locationClass);
        ...
        LOG.info("XL-NameNodeService::createFile.getBlock for " + request.getFileName() + ", storageClass = " + fileInfo.getStorageClass() + ", locationClass = " + fileInfo.getLocationClass());
        
        // XL hack for openec
        boolean linkoec = CrailConstants.LINK_OEC;
        NameNodeBlockInfo fileBlock = null;
        if (linkoec)
            fileBlock = blockStore.getBlockFromOEC(fileHash.toString(), storageClass);
        if (fileBlock == null)
            fileBlock = blockStore.getBlock(fileInfo.getStorageClass(), fileInfo.getLocationClass());
        ...
        LOG.info("XL-NameNodeService::diroffset = " + fileInfo.getDirOffset());
		if (fileInfo.getDirOffset() >= 0){
			int index = CrailUtils.computeIndex(fileInfo.getDirOffset());
			parentBlock = parentInfo.getBlock(index);
			if (parentBlock == null){
                LOG.info("XL-NameNodeService::createFile.getBlock for parent, storageClass = " + parentInfo.getStorageClass() + ", locationClass = " + parentInfo.getLocationClass());
